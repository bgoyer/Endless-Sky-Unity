...
namespace Assets.Scripts.AI
{
    public class AI_Attack : StateMachineBehaviour
    {
        ...
        private void attack(Animator animator)
        {
            ...
            {
                game.RotateTowards(ship, -(ship.transform.position - target.transform.position).normalized);
                rd2.drag = .3f;
                game.StopThruster(ship);
            }
            else
            {
                game.Accelerate(ship);
